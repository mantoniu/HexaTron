...
export class GameHeader extends ListenerComponent {
    ...
    calculateUtils() {
        ...
        const pxSize = convertRemToPixels(15);
        this.context.font = pxSize + "px serif";
        const size = width / 7;//this.context.measureText("2").fontBoundingBoxAscent - this.context.measureText("2").fontBoundingBoxDescent;
    ...
    draw() {
        const [height, size, position] = this.calculateUtils();
        for (let k = 1; k < 4; k++) {
            this.drawHexagon(k, "#ffffff", false);
            if (k !== 3) {
                this.context.beginPath();
